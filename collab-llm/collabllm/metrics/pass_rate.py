...
class PassRate(MultiturnMetric):
    ...
    def __call__(self, single_turn_data, chat_eval, final_answer=None, **kwargs):
        ...
        # check_correctness_bcb
        Returns:
            dict: a dictionary containing the bert score precision, recall, and f1
        '''
        problem = {'prompt': single_turn_data[0]['content']}
        problem.update(json.loads(single_turn_data[-1]['content']))
...
# The functions below are from OpenAI's HumanEval github repository.
def check_correctness(problem: Dict, completion: str, timeout: float,
                      completion_id: Optional[int] = None, add_prompt = True) -> Dict:
    """
