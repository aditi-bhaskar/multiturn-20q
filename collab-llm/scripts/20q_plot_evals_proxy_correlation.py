...
# base_folder = "/Users/aditi/Documents/multiturn-20q/collab-llm/outputs/eval/20q/local20q/test/4-PAPER_ACTUAL_MERGED_FINAL"


# #  THIS ONE ADDS LABELS TO THE OBJECTS WITH ACC = 1
...
#     # Removed all point annotations here
...
def plot_corr(x, y, labels, x_label, y_label, title, save_path):
    ...
    coords = list(zip(x, y))
    freq = Counter(coords)
    sizes = [10 * freq[pt] for pt in coords]

    plt.figure(figsize=(8, 6))
    plt.scatter(x, y, s=sizes, color='blue', alpha=0.7, edgecolor='k')
...
def process_folder(folder_path):
    ...
    plot_corr(accuracy_scores, info_gain_scores, targets,
              "Accuracy Score", "Information Gain Score",
              f"{title} — Accuracy vs Info Gain",
              os.path.join(folder_path, "plot_corr_accuracy_infogain.png"))

    plot_corr(accuracy_scores, interactivity_scores, targets,
              "Accuracy Score", "Interactivity Score",
              f"{title} — Accuracy vs Interactivity",
              os.path.join(folder_path, "plot_corr_accuracy_interactivity.png"))

    plot_corr(info_gain_scores, interactivity_scores, targets,
              "Information Gain Score", "Interactivity Score",
              f"{title} — Info Gain vs Interactivity",
              os.path.join(folder_path, "plot_corr_infogain_interactivity.png"))
